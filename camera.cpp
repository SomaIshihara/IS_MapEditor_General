...
//静的メンバ変数
const float CCamera::LENGTH = 20000.0f;		//カメラが見える最大距離
const float CCamera::ROT_X_MIN = -0.45f;	//カメラのX角度の最低値[rad]
const float CCamera::ROT_X_MAX = -0.2f;		//カメラのX角度の最低値[rad]
...
void CCamera::SetCamera(void)
{
	...
	//作成
	D3DXMatrixPerspectiveFovLH(&m_mtxProjection, D3DXToRadian(45.0f), (float)SCREEN_WIDTH / (float)SCREEN_HEIGHT, 10.0f, LENGTH);
