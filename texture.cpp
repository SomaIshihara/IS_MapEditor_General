		...
		while (1)
		{
			...
			{//読み取り
				if (bLoad == true)
				{
					...
					{
						LoadTexture(&aPath[0], nPatWidth, nPatHeight);
					}
				...
				else if (strncmp(&aCode[0], CODE_START_LOAD, sizeof CODE_START_LOAD - 1) == 0)
				{
					bLoad = true;
				}
...
int CTexture::Regist(const char * pPath, const int nPatWidth, const int nPatHeight)
{
	...
	//新規登録
	if (m_nNumAll < MAX_TEXTURE && SUCCEEDED(LoadTexture(pPath, nPatWidth, nPatHeight)))
	{//テクスチャ保存限界以内で読み込めた
		return m_nNumAll - 1;
	}
	else
...
//読み込み（共通部）
//=================================
HRESULT CTexture::LoadTexture(const char * pPath, const int nPatWidth, const int nPatHeight)
{
	//デバイス取得
	LPDIRECT3DDEVICE9 pDevice = CManager::GetRenderer()->GetDevice();

	//テクスチャ読み込み
	if (SUCCEEDED(D3DXCreateTextureFromFile(pDevice, pPath,
		&m_apTexture[m_nNumAll].pTexture)))
	{
		//ファイルパスコピー
		int nLength = strlen(pPath) + 1;
		m_aFilepath[m_nNumAll] = new char[nLength];
