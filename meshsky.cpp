...
HRESULT CMeshSky::Init(void)
{
	...
	//頂点バッファの生成
	pDevice->CreateVertexBuffer(sizeof(VERTEX_3D) * ((m_nBlockWidth) * (m_nBlockDepth + 1) + 2),
		D3DUSAGE_WRITEONLY,
		FVF_VERTEX_3D,
		D3DPOOL_MANAGED,
		&m_pVtxbuff,
	...
	//インデックスバッファの生成
	pDevice->CreateIndexBuffer(sizeof(WORD) * (m_nBlockDepth + 2),
		D3DUSAGE_WRITEONLY,
		D3DFMT_INDEX16,
		D3DPOOL_MANAGED,
		&m_pIdxBuffTop,
	...
	//頂点座標+テクスチャ座標
	for (int cntV = 0; cntV < m_nBlockWidth; cntV++)
	{
		float fAngleV = (1.0f * D3DX_PI) * ((float)(cntV + 1) / (m_nBlockWidth + 1));
		float fTexV = (1.0f / (m_nBlockWidth + 2)) * cntV;

		for (int cntH = 0; cntH < m_nBlockDepth + 1; cntH++)
		{
			float fAngleH = (2.0f * D3DX_PI) * ((float)cntH / (m_nBlockDepth));
			float fTexU = (1.0f / (m_nBlockDepth + 1)) * cntH;
			int cnt = cntV * (m_nBlockDepth + 1) + cntH + 1;
	...
	//下部
	int nLastVtx = (m_nBlockWidth) * (m_nBlockDepth + 1) + 1;
	...
	for (int cnt = 0; cnt < m_nBlockDepth + 1; cnt++)
	{
		pIdx[cnt] = cnt;
	}
	pIdx[m_nBlockDepth + 1] = 1;
	...
	for (nCntWidth = 0; nCntWidth < m_nBlockDepth - 1; nCntWidth++)
	{
		...
		*pIdx = (m_nBlockWidth + 1) + (m_nBlockWidth + 1) * (nCntWidth + 1) + 1;
		pIdx++;
	}
	...
	int nVtxNum = (m_nBlockWidth) * (m_nBlockDepth + 1) + 2;

	for (int cnt = 0; cnt < m_nBlockDepth + 1; cnt++)
	{
		pIdx[cnt] = nVtxNum - cnt - 1;
	}
	pIdx[m_nBlockDepth + 1] = nVtxNum - 2;
...
void CMeshSky::Draw(void)
{
	...
	//ポリゴン描画（インデックスされたやつ）
	pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLEFAN, 0, 0, (m_nBlockDepth + 1), 0, (m_nBlockDepth));
	...
	//ポリゴン描画（インデックスされたやつ）
	int nPolyNum = (((m_nBlockWidth * m_nBlockDepth * 2) + (2 * (m_nBlockDepth - 1)))) - 2;
	pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLESTRIP, 0, 0, (m_nBlockWidth - 1) * (m_nBlockDepth + 1) + m_nBlockDepth, 0, nPolyNum);
}
...
WORD* CMeshSky::SetIdxSwaingField(int nCntWidth, WORD *pIdx)
{
	...
	{
		*pIdx = (m_nBlockWidth + 1) * ((nCountHeight % 2) ^ 0x1) + nCountHeight / 2 + (m_nBlockWidth + 1) * nCntWidth + 1;
	}
	return pIdx;
}
